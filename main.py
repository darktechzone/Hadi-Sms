...
# ---------- FULL COUNTRY MAP (complete, same as JS) ----------
COUNTRY_MAP = {
...
# ---------- SESSION MANAGEMENT (EXACTLY LIKE JS) ----------
            ...
            for login_path in login_paths:
                try:
                    ...
                    if not captcha_match:
                        print("[LOGIN] No captcha found – skipping this login path")
                        continue
...
def ensure_session():
    ...
    if not session or (time.time() - last_login) > 3600:
        print("[SESSION] Expired or missing – re-logging...")
        return login()
    if sesskey is None:
        try:
            r = session.get(
            ...
            if r.status_code == 200:
                m = re.search(r'sesskey=([^&\s"\']+)', r.text)
                if m:
                    sesskey = m.group(1)
...
# ---------- FETCH FUNCTIONS WITH RETRY & CIRCUIT BREAKER ----------
    ...
    while attempt < retries:
        try:
            ...
            if resp.status_code in (503, 429, 403):
                ...
                print(f"[FETCH] {resp.status_code}, attempt {attempt+1}/{retries}, waiting {wait}s...")
                time.sleep(wait)
            ...
            if attempt < retries:
                time.sleep(delay)
                delay = min(delay * 2, 10)
    print(f"[FETCH] Max retries exceeded for {url}")
    return None

def fetch_with_circuit_breaker(url, method='GET', headers=None, params=None, data=None):
    global consecutive_failures
    if consecutive_failures >= FAILURE_THRESHOLD:
        print(f"[CIRCUIT] Skipping fetch for {BREAKER_TIMEOUT}s")
...
def fetch_numbers():
    ...
    if not resp or resp.status_code != 200:
        raise Exception(f"Numbers fetch failed: {resp.status_code if resp else 'no response'}")
    data = resp.json()
...
def fetch_otps(limit=10):
    ...
    # The client panel may use a different parameter set; we keep the same as agent but with new endpoint
    params = {
    ...
    resp = fetch_with_circuit_breaker(url, 'GET', headers, params)
    if not resp or resp.status_code != 200:
        print(f"[OTP] Fetch failed: {resp.status_code if resp else 'no response'}")
        return []
    data = resp.json()
    if not data.get('aaData'):
...
def root():
    return jsonify({
        "message": "NumberPanel API (Client Account)",
