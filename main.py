...
def login():
    ...
    with login_lock:
        ...
        try:
            ...
            # Fetch sesskey from /agent/SMSCDRStats (still works for client)
            time.sleep(0.5)
            r3 = session.get(
            ...
            if r3.status_code == 200:
                ...
                for pat in patterns:
                    ...
                    if m:
                        ...
                        break
                if sesskey is None:
                    add_log("Sesskey not found in response")

            if not validate_session():
                add_log("Session validation failed")
                raise Exception("Session validation failed")
            add_log("Login complete and validated")
            return True
        except Exception as e:
    ...
    if sesskey is None:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if m:
                    sesskey = m.group(1)
                    add_log(f"Sesskey refreshed: {sesskey}")
        except:
            pass
        if sesskey is None:
            add_log("Sesskey missing and could not refresh – forcing re-login")
            return login()
    return True
...
def validate_session():
    try:
        ...
        if resp.status_code == 200 and resp.json().get("aaData") is not None:
            add_log("Session validation OK")
            return True
        add_log(f"Session validation failed: status {resp.status_code}")
        return False
    except Exception as e:
...
# ---------- FETCH NUMBERS (original /agent/res/ endpoints) ----------
def fetch_numbers():
...
# ---------- FETCH OTPs (original /agent/res/ endpoints) ----------
...
def root():
    return jsonify({
        "message": "NumberPanel API (Client – original /res/ endpoints)",
