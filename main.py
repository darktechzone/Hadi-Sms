...
def login():
    ...
    with login_lock:
        if session and (time.time() - last_login) < 86400:  # 24h check – but validation overrides
            # Quick test to see if session is still alive
            try:
                test_url = f"{BASE_URL}/agent/res/data_smsnumbers.php"
                ...
                pass
            # If test fails, fall through to re‑login
        try:
...
def ensure_session():
    ...
    # If session object exists, test it first
    if session:
        try:
            test_url = f"{BASE_URL}/agent/res/data_smsnumbers.php"
            params = {"sEcho": "1", "iDisplayStart": "0", "iDisplayLength": "1"}
            ...
            resp = session.get(test_url, headers=HEADERS, params=params, timeout=5)
            if resp.status_code == 200 and resp.json().get("aaData") is not None:
                # Session is valid – update last_login timestamp to keep it fresh
                last_login = time.time()
                return True
        except:
            pass
        # If test fails, session is dead – re‑login
        print("[SESSION] Invalid – re‑logging...")
    else:
        print("[SESSION] No session – logging in...")
    return login()
